...
class ResNet_3d(fluid.dygraph.Layer):
...
if __name__ == '__main__':
    with fluid.dygraph.guard():
        network = ResNet_3d(50)
        inner_state_dict = network.state_dict()
        print(inner_state_dict.keys())
        for name, para in inner_state_dict.items():
             print(name)
        img = np.zeros([1, 1, 48, 112, 112]).astype('float32')
